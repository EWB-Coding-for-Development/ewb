...
def play_wav(filename):
    with open(os.devnull, "w") as dev_null:
        subprocess.call(["ffmpeg","-i",filename,"-f","s16le","-acodec","pcm_s16le","-ac", "2" if play_stereo else "1" ,"-ar","22050","-"],stdout=music_pipe_w, stderr=dev_null)
...
def run_pifm(use_audio_in=False, sample_rate=22):
    global fm_process
    #with open(os.devnull, "w") as dev_null:
    try:
        fm_process = subprocess.Popen(["pifm","-",str(frequency),str(sample_rate), "stereo" if play_stereo else "mono"], stdin=music_pipe_r, stdout=sys.stdout)
    except OSError as e:
        raise e
...
def say_radio(say_text, **kwargs):
    say(say_text, stdout=music_pipe_w, **kwargs)
